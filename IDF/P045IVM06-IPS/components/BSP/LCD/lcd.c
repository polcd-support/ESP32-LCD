...
static inline void lcd_rgb565_to_rgb888(uint16_t color, uint8_t *rgb)
{
    rgb[0] = (uint8_t)((((color >> 11) & 0x1F) * 255) / 31);
    rgb[1] = (uint8_t)((((color >> 5) & 0x3F) * 255) / 63);
    rgb[2] = (uint8_t)(((color & 0x1F) * 255) / 31);
}

static void lcd_fill_rgb888(uint8_t *buffer, uint32_t pixel_count, uint16_t color)
{
    uint8_t rgb[LCD_COLOR_BYTES];
    lcd_rgb565_to_rgb888(color, rgb);

    for (uint32_t i = 0; i < pixel_count; i++)
    {
        buffer[i * LCD_COLOR_BYTES] = rgb[0];
        buffer[i * LCD_COLOR_BYTES + 1] = rgb[1];
        buffer[i * LCD_COLOR_BYTES + 2] = rgb[2];
    }
}
...
{
    uint8_t *buffer = (uint8_t *)lcd_buffer[buffer_sw];
    lcd_fill_rgb888(buffer, lcddev.width * lcddev.height, color);
    lcd_set_back_color(color);
...
void lcd_draw_point(uint16_t x, uint16_t y, uint16_t color)
{
    uint8_t rgb[LCD_COLOR_BYTES];
    lcd_rgb565_to_rgb888(color, rgb);
    esp_lcd_panel_draw_bitmap(lcddev.lcd_panel_handle, x, y, x + 1, y + 1, rgb);
}

void lcd_draw_rgb565_bitmap(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint16_t *data)
{
    if (data == NULL || width == 0 || height == 0 || x >= lcddev.width || y >= lcddev.height ||
        (x + width) > lcddev.width || (y + height) > lcddev.height)
    {
        ESP_LOGE("TAG", "Invalid bitmap area");
        return;
    }

    uint8_t *line = heap_caps_malloc(width * LCD_COLOR_BYTES, MALLOC_CAP_INTERNAL);
    if (line == NULL)
    {
        ESP_LOGE("TAG", "Memory for bitmap line is not enough");
        return;
    }

    for (uint16_t row = 0; row < height; row++)
    {
        const uint16_t *src = &data[row * width];
        for (uint16_t col = 0; col < width; col++)
        {
            lcd_rgb565_to_rgb888(src[col], &line[col * LCD_COLOR_BYTES]);
        }

        esp_lcd_panel_draw_bitmap(lcddev.lcd_panel_handle, x, y + row, x + width, y + row + 1, line);
    }
...
/**
 * @brief       在指定区域内填充单个颜色
 * @note        输入颜色为RGB565,内部转换为RGB888格式发送.
...
void lcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t color)
{
    ...
    /* 分配内存 */
    uint8_t *buffer = heap_caps_malloc(width * LCD_COLOR_BYTES, MALLOC_CAP_INTERNAL);
    ...
    {
        lcd_fill_rgb888(buffer, width, color);
...
void lcd_color_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
    ...
    for (uint16_t y_index = 0; y_index < height; y_index++)
    {
        ...
        {
            lcd_rgb565_to_rgb888(color[buf_index], &buffer[x_index * LCD_COLOR_BYTES]);
            buf_index++;
        }
...
void lcd_draw_hline(uint16_t x, uint16_t y, uint16_t len, uint16_t color)
{
    ...
    uint16_t ex = fmin(lcddev.width, x + len);
    lcd_fill(x, y, ex, y + 1, color);
}
...
void lcd_show_char(uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    if (mode == 0)
    {
        uint16_t draw_w = char_width;
        uint16_t draw_h = size;

        if (x >= lcddev.width || y >= lcddev.height)
        {
            return;
        }
        if ((x + draw_w) > lcddev.width)
        {
            draw_w = (uint16_t)(lcddev.width - x);
        }
        if ((y + draw_h) > lcddev.height)
        {
            draw_h = (uint16_t)(lcddev.height - y);
        }

        uint8_t *char_buf = heap_caps_malloc(draw_w * draw_h * LCD_COLOR_BYTES, MALLOC_CAP_INTERNAL);
        if (char_buf != NULL)
        {
            uint8_t fg[LCD_COLOR_BYTES];
            uint8_t bg[LCD_COLOR_BYTES];

            lcd_rgb565_to_rgb888(color, fg);
            lcd_rgb565_to_rgb888((uint16_t)g_back_color, bg);

            for (uint16_t i = 0; i < (draw_w * draw_h); i++)
            {
                char_buf[i * LCD_COLOR_BYTES] = bg[0];
                char_buf[i * LCD_COLOR_BYTES + 1] = bg[1];
                char_buf[i * LCD_COLOR_BYTES + 2] = bg[2];
            }
            ...
            for (t = 0; t < csize; t++)
            {
                ...
                for (t1 = 0; t1 < 8; t1++)
                {
                    uint16_t px = (uint16_t)(t / ((size / 8) + ((size % 8) ? 1 : 0)));
                    uint16_t py = (uint16_t)(((t % ((size / 8) + ((size % 8) ? 1 : 0))) * 8) + t1);

                    if (px < draw_w && py < draw_h && (temp & 0x80))
                    {
                        uint32_t idx = ((py * draw_w) + px) * LCD_COLOR_BYTES;
                        char_buf[idx] = fg[0];
                        char_buf[idx + 1] = fg[1];
                        char_buf[idx + 2] = fg[2];
                    }
            ...
            esp_lcd_panel_draw_bitmap(lcddev.lcd_panel_handle, x, y, x + draw_w, y + draw_h, char_buf);
