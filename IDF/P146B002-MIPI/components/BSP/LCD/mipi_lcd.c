...
esp_lcd_panel_handle_t mipi_lcd_init(void)
{
    ...
    if (mipidev.id != 0x8394 && mipidev.id != 0x8399 && mipidev.id != 0x9881 && mipidev.id != MIPI_LCD_ID_JD9855)
    {
        ESP_LOGW(mipi_lcd_tag, "unknown panel id:%#x, use JD9855 as default", mipidev.id);
        mipidev.id = MIPI_LCD_ID_JD9855;
    }
    ...
    if (mipidev.id == MIPI_LCD_ID_JD9855)                       /* 1.46寸360*360屏幕 */
    {
    ...
    ESP_LOGI(mipi_lcd_tag, "dsi: lanes=%u, lane_bitrate=%uMbps, bpp=%u, pixel_format=RGB565",
             MIPI_DSI_LANE_NUM, MIPI_DSI_LANE_BITRATE_MBPS, 16);
