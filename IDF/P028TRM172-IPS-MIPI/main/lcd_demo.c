    ...
    while (1) {
        ...
        case STATE_TEXT:
            if (!state_drawn) {
                ...
                lcd_show_string(28, 190, 420, 32, 24, "480 x 640 RGB888", CYAN);
                lcd_show_string(28, 230, 420, 32, 24, "Touch IC: FT6336U", YELLOW);
                state_drawn = true;
            }
            if (elapsed_ms >= 2200) {
        ...
        case STATE_HANDWRITING: {
            ...
            if (ft6336_read(&touch)) {
                ...
                uint16_t pen_x = x;
                uint16_t pen_y = y;
                if (pen_x >= lcddev.width) {
                    pen_x = (uint16_t)(lcddev.width - 1);
                }
                if (pen_y >= lcddev.height) {
                    pen_y = (uint16_t)(lcddev.height - 1);
                }
